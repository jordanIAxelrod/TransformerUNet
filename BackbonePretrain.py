...
class Head(nn.Module):
    def __init__(self, backbone, depth, activation, n_classes):
        ...
        self.input = nn.Linear(32 * 32 * CHAN_LIST[-1], 32 * 32 * CHAN_LIST[-1] * 5)
        self.hidden = nn.ModuleList(
            [
                nn.Linear(32 * 32 * CHAN_LIST[-1] * 5, 32 * 32 * CHAN_LIST[-1] * 5)
...
net = Head(net_backbone, 5, nn.ReLU, 1000)
transform = A.Compose([
